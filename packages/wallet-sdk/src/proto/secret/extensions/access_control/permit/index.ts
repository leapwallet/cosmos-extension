...
import { serializeStdSignDoc, StdSignDoc } from '@leapwallet/leap-keychain';
import { sha256 } from '@noble/hashes/sha256';
import * as secp256k1 from '@noble/secp256k1';
import { bech32 } from 'bech32';

import { base64PubkeyToAddress, PubKey, StdSignature, stringToCoins } from '../../../index';
  ...
  if (!keplr) {
    ...
    signature =
      typeof signer.signPermit === 'function'
        ? (await signer.signPermit(owner, newSignDoc(chainId, permitName, allowedTokens, permissions))).signature
        : (await signer.signAmino(owner, newSignDoc(chainId, permitName, allowedTokens, permissions))).signature;
  }
  //@ts-ignore
...
export const validatePermit = (
  ...
  const contractInPermit = permit.params.allowed_tokens.includes(contract);
  ...
  const permissionInPermit = permit.params.permissions.find((p) => permissions.includes(p));
...
const _validate_sig = (permit: Permit): boolean => {
  ...
  const sig = secp256k1.Signature.fromCompact(fromBase64(permit.signature.signature));

  return secp256k1.verify(sig, messageHash, fromBase64(permit.signature.pub_key.value));
};
