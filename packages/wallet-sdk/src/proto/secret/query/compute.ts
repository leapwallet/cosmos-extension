...
export class ComputeQuerier {
  ...
  async codeHashByCodeId(req: QueryByCodeIdRequest, headers?: HeadersInit): Promise<QueryCodeHashResponse> {
  ...
  async queryContract<T extends object, R extends any>(
    { contract_address: contractAddress, code_hash: codeHash, query }: QueryContractRequest<T>,
    headers?: HeadersInit,
  ): Promise<R> {
    if (!codeHash) {
    ...
    try {
      ...
      try {
        ...
        const decryptedBase64Error = await this.encryption!.decrypt(encryptedError, nonce);
  ...
  async contractHistory(
    ...
    const decryptedEntries: ContractCodeHistoryEntry[] = [];
    for (const entry of entries ?? []) {
      ...
      try {
        ...
        const plaintextInitMsg = await this.encryption!.decrypt(encryptedInitMsg, nonce);
