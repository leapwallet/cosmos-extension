...
export class Service {
  ...
  static GetTx(req: GetTxRequest, initReq?: fm.InitReq): Promise<GetTxResponse> {
    return fm.fetchReq<GetTxRequest, GetTxResponse>(
      `/cosmos/tx/v1beta1/txs/${req['hash']}?${fm.renderURLSearchParams(req, ['hash'])}`,
      { ...initReq, method: 'GET' },
    );
  }
  ...
  static GetTxsEvent(req: GetTxsEventRequest, initReq?: fm.InitReq): Promise<GetTxsEventResponse> {
    return fm.fetchReq<GetTxsEventRequest, GetTxsEventResponse>(
      `/cosmos/tx/v1beta1/txs?${fm.renderURLSearchParams(req, [])}`,
      { ...initReq, method: 'GET' },
    );
  }
