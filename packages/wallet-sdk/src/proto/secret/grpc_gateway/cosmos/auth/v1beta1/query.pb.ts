...
export class Query {
  static Accounts(req: QueryAccountsRequest, initReq?: fm.InitReq): Promise<QueryAccountsResponse> {
    return fm.fetchReq<QueryAccountsRequest, QueryAccountsResponse>(
      `/cosmos/auth/v1beta1/accounts?${fm.renderURLSearchParams(req, [])}`,
      { ...initReq, method: 'GET' },
    );
  }
  ...
  static Params(req: QueryParamsRequest, initReq?: fm.InitReq): Promise<QueryParamsResponse> {
    return fm.fetchReq<QueryParamsRequest, QueryParamsResponse>(
      `/cosmos/auth/v1beta1/params?${fm.renderURLSearchParams(req, [])}`,
      { ...initReq, method: 'GET' },
    );
  }
  static ModuleAccountByName(
    req: QueryModuleAccountByNameRequest,
    initReq?: fm.InitReq,
  ): Promise<QueryModuleAccountByNameResponse> {
    return fm.fetchReq<QueryModuleAccountByNameRequest, QueryModuleAccountByNameResponse>(
      `/cosmos/auth/v1beta1/module_accounts/${req['name']}?${fm.renderURLSearchParams(req, ['name'])}`,
      { ...initReq, method: 'GET' },
    );
  }
}
