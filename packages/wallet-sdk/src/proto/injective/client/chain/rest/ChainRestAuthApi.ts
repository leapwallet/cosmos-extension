import { HttpRequestException, UnspecifiedErrorCode } from '../../../exceptions/src';
import BaseRestConsumer from '../../base/BaseRestConsumer';
import { ChainModule, RestApiResponse } from '../types';
import { AccountResponse, BaseAccountRestResponse, CosmosAccountRestResponse } from './../types/auth-rest';
...
export class ChainRestAuthApi extends BaseRestConsumer {
   ...
   */
  public async fetchAccount(address: string, params: Record<string, any> = {}): Promise<AccountResponse> {
    const endpoint = `cosmos/auth/v1beta1/accounts/${address}`;

    try {
      const response = await this.retry<RestApiResponse<AccountResponse>>(() => this.get(endpoint, params));
   ...
   * @param address address of account to look up
   */
  public async fetchCosmosAccount(address: string, params: Record<string, any> = {}): Promise<BaseAccountRestResponse> {
    const endpoint = `cosmos/auth/v1beta1/accounts/${address}`;

    try {
      const isInjectiveAddress = address.startsWith('inj') || address.startsWith('evmos');

      const response = await this.retry<RestApiResponse<AccountResponse | CosmosAccountRestResponse>>(() =>
