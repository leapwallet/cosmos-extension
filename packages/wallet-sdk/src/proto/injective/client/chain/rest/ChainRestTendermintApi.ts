...
export class ChainRestTendermintApi extends BaseRestConsumer {
  async fetchLatestBlock(params: Record<string, any> = {}): Promise<BlockLatestRestResponse['block']> {
    const endpoint = `cosmos/base/tendermint/v1beta1/blocks/latest`;

    try {
      const response = await this.retry<RestApiResponse<BlockLatestRestResponse>>(() => this.get(endpoint, params));
  ...
  async fetchNodeInfo(params: Record<string, any> = {}): Promise<{
    ...
    try {
      const response = await this.retry<RestApiResponse<NodeInfoRestResponse>>(() => this.get(endpoint, params));
