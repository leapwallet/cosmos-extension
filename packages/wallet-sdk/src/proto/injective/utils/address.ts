 ...
 * @returns string
 */
export const getInjectiveAddressFromSubaccountId = (subaccountId: string): string => {
 ...
 * @returns string
 */
export const getSubaccountId = (injectiveAddress: string, nonce = 0): string => {
  return `0x${Buffer.from(bech32.fromWords(bech32.decode(injectiveAddress).words)).toString('hex')}${'0'.repeat(
    23,
  )}${nonce}`;
};
...
export const getChecksumAddress = (ethAddress: string) => {
  ...
  const addressHash = keccak256(Buffer.from(lowercasedAddress)).toString('hex').replace('0x', '');
...
export const isCw20ContractAddress = (address: string) => address.length === 42 && address.startsWith('inj');
