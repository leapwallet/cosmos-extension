...
import { ErrorContext, ErrorContextCode, TransactionChainErrorModule, UnspecifiedErrorCode } from '../types';
...
export const mapFailedTransactionMessageFromString = (
  ...
  const messageInMapKey = (Object.keys(chainErrorMessagesMap) as Array<keyof typeof chainErrorMessagesMap>).find(
    (key) => parsedMessage.toLowerCase().includes(key.toLowerCase()),
  );
