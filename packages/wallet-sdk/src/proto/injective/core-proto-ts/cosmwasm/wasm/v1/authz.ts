...
export const MaxCallsLimit = {
  ...
  fromPartial(object: Partial<MaxCallsLimit>): MaxCallsLimit {
    const message = createBaseMaxCallsLimit();
    message.remaining =
      object.remaining !== undefined && object.remaining !== null ? BigInt(object.remaining.toString()) : BigInt(0);
    return message;
  },
...
export const Cosmwasm_wasmv1ContractAuthzFilterX_InterfaceDecoder = (
  input: BinaryReader | Uint8Array,
): AllowAllMessagesFilter | AcceptedMessageKeysFilter | AcceptedMessagesFilter | Any => {
  const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
  const data = Any.decode(reader, reader.uint32());
  switch (data.typeUrl) {
    ...
    case 'wasm/AllowAllMessagesFilter':
      return Any.fromPartial({
        ...
        value: AllowAllMessagesFilter.encode(
          AllowAllMessagesFilter.fromPartial(AllowAllMessagesFilter.fromAmino(content.value)),
        ).finish(),
      });
    case 'wasm/AcceptedMessageKeysFilter':
      return Any.fromPartial({
        ...
        value: AcceptedMessageKeysFilter.encode(
          AcceptedMessageKeysFilter.fromPartial(AcceptedMessageKeysFilter.fromAmino(content.value)),
        ).finish(),
      });
    case 'wasm/AcceptedMessagesFilter':
      return Any.fromPartial({
        ...
        value: AcceptedMessagesFilter.encode(
          AcceptedMessagesFilter.fromPartial(AcceptedMessagesFilter.fromAmino(content.value)),
        ).finish(),
      });
    default:
      return Any.fromAmino(content);
