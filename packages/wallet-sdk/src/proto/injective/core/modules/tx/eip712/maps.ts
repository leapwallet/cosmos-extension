...
export const objectKeysToEip712Types = ({
  ...
  for (const property in convertToSnakeCase(object)) {
    const propertyValue = convertToSnakeCase(object)[property];
    ...
    if (type === 'boolean') {
    ...
    } else if (type === 'number' || type === 'bigint' || numberFieldsWithStringValue.includes(property)) {
      types.push({
        name: property,
      ...
      if (Array.isArray(propertyValue) && propertyValue.length === 0) {
        ...
        const isPrimitive = arrayFirstType === 'boolean' || arrayFirstType === 'number' || arrayFirstType === 'string';
...
export const mapValuesToProperValueType = <T extends Record<string, unknown>>(
  ...
  const sdkDecKeys = ['min_price_tick_size', 'price', 'quantity', 'margin', 'trigger_price', 'min_quantity_tick_size'];
  ...
  return Object.keys(object).reduce((result, key) => {
    ...
    if (!value) {
      ...
      if (messageTypeUrl) {
        ...
        if (typeInMap) {
          const nullableStringKeys = nullableStringsTypeMaps[typeInMap as keyof typeof nullableStringsTypeMaps];
      ...
      if (Array.isArray(value)) {
        return {
          ...
          [key]: value.every((i) => typeof i === 'string')
            ? value
            : value.map((item) => mapValuesToProperValueType(item as Record<string, unknown>)),
        };
      }
    ...
    if (typeof value === 'string') {
      ...
      if (messageTypeUrl) {
        ...
        if (typeInMap) {
          const sdkDecKeys = sdkDecKeyWithTypeMaps[typeInMap as keyof typeof sdkDecKeyWithTypeMaps];
 ...
 */
const appendWasmTypePrefixToPropertyType = (property: string, parentProperty: string = '') => {
 ...
 * including its parent property type
 */
const appendTypePrefixToPropertyType = (property: string, parentProperty: string = '') => {
  const propertyWithoutTypePrefix = property.replace('Type', '');
