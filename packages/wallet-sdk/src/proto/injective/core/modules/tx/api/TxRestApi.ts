...
import { BigNumberInBase, HttpClient } from '../../../../utils/classes';
import {
...
import { BroadcastMode, SimulationResponse, TxInfoResponse, TxResultResponse } from '../types/tx-rest-client';
...
export class TxRestApi implements TxConcreteApi {
  ...
  public async fetchTx(txHash: string, params: any = {}): Promise<TxResponse> {
    try {
      const response = await this.getRaw<TxResultResponse>(`/cosmos/tx/v1beta1/txs/${txHash}`, params);
  ...
  public async fetchTxPoll(
    ...
    throw new HttpRequestException(
      new Error(`Transaction was not included in a block before timeout of ${timeout}ms`),
      {
        context: 'TxRestApi',
  ...
  public async simulate(txRaw: CosmosTxV1Beta1Tx.TxRaw) {
    ...
    try {
      const response = await this.postRaw<SimulationResponse>('/cosmos/tx/v1beta1/simulate', {
  ...
  public async broadcast(tx: CosmosTxV1Beta1Tx.TxRaw, options?: TxClientBroadcastOptions): Promise<TxResponse> {
    const timeout =
      options?.timeout ||
  ...
  private async broadcastTx<T>(txRaw: CosmosTxV1Beta1Tx.TxRaw, mode: BroadcastMode = BroadcastMode.Sync): Promise<T> {
  ...
  private async getRaw<T>(endpoint: string, params: URLSearchParams | any = {}): Promise<T> {
    try {
      return await this.httpClient.get<URLSearchParams | any, { data: T }>(endpoint, params).then((d) => d.data);
    } catch (e) {
  ...
  private async postRaw<T>(endpoint: string, params: URLSearchParams | any = {}): Promise<T> {
    try {
      return await this.httpClient.post<URLSearchParams | any, { data: T }>(endpoint, params).then((d) => d.data);
    } catch (e) {
      const error = e as Error | AxiosError;
