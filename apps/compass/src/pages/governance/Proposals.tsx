...
const GeneralProposals = observer(({ governanceStore }: ProposalsProps) => {
...
const NeutronProposals = observer(({ governanceStore }: ProposalsProps) => {
  const [selectedProposal, setSelectedProposal] = useState<string | undefined>()
...
function Proposals() {
  ...
  if (activeChain === AGGREGATED_CHAIN_KEY) {
    return <AggregatedGovernance governanceStore={governanceStore} />
  }

  if (isGovernanceComingSoon) {
    return <ComingSoon title='Governance' />
  }

  return activeChain === 'neutron' ? (
    <NeutronProposals governanceStore={governanceStore} />
  ) : (
    <GeneralProposals governanceStore={governanceStore} />
  )
}
