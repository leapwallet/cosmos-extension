...
export function useGetSourceAssets(sourceChain: SourceChain | undefined) {
  ...
  const autoFetchedCW20Tokens = useAutoFetchedCW20Tokens(sourceChain?.key ?? undefined)
  const combinedDenoms = { ...denoms, ...autoFetchedCW20Tokens }
  const [preferredCurrency] = useUserPreferredCurrency()
  const { ibcTraceData } = useIbcTraceStore()
  const chainInfos = useChainInfos()
  const selectedNetwork = useSelectedNetwork()
  useInitializeCW20TokensForChain(sourceChain)
  ...
  return useQuery(
    ...
    async function () {
      if (sourceAssets && sourceAssets.success && sourceAssets.assets.length > 0) {
        ...
        for (const skipAsset of sourceAssets.assets) {
          ...
          if (token) {
            ...
            if (denomInfo) {
              ...
              if (skipAsset.denom.includes('ibc/')) {
                ...
                if (trace) {
                  ...
                  needToFetchIbcSourceChainsFor.push({
                    ...
                    chain_id: String(sourceChain?.chainId ?? ''),
                  })
                }
              ...
              _sourceAssets.push(asset as SourceToken)
            }
          }
        }
        ...
        try {
          ...
          needToFetchIbcSourceChainsFor.forEach((ibcAsset, index) => {
            const destinationAsset = _sourceAssets.find(
        ...
        return {
          assets: sortTokenBalances(_sourceAssets).sort(
