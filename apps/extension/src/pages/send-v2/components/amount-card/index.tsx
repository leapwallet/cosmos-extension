...
export const AmountCard = React.memo(({ allAssets, isAllAssetsLoading }: AmountCardProps) => {
  ...
  const isTokenStatusSuccess = useMemo(() => {
    ...
    return status
  }, [addressLinkState, isAllAssetsLoading, seiEvmStatus])
  ...
  useEffect(() => {
    if (assetCoinDenom) {
      const tokenFromParams: Token | null =
        assets.find(
          (asset) => asset.ibcDenom === assetCoinDenom || asset.coinMinimalDenom === assetCoinDenom,
        ) || null
