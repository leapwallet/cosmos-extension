...
export default function SwitchChain() {
  ...
  const [requestedChain, setRequestedChain] = useState<string>('solana:mainnet')
  ...
  useEffect(() => {
    Browser.storage.local
      ...
      .then(async function initializeContractInfo(response) {
        const { network, url, origin } = response[REDIRECT_REQUEST].payload
        setRequestedChainId(network)
        setExistingChainId(url)
        setOrigin(origin)
      })
  ...
  const handleSwitchChainClick = async () => {
    ...
    const storageKey = getChainOriginStorageKey(origin)
        ...
        >
          <div className='px-7 py-3 overflow-y-auto relative h-[450px]'>
            <h2 className='text-center text-lg font-bold dark:text-white-100 text-gray-900 w-full'>
              Allow this site to switch the chain?fjsdlfhjskdfj
            </h2>
